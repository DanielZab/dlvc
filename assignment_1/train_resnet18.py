...
def train(args):
    ...
    train_data = CIFAR10Dataset(fdir, Subset.TRAINING, transform=train_transform)

    val_data = CIFAR10Dataset(fdir, Subset.VALIDATION, transform=val_transform)

    device = "cuda"

    model = DeepClassifier(resnet18(num_classes=train_data.num_classes()))
    ...
    lr_scheduler = torch.optim.lr_scheduler.LinearLR(optimizer=optimizer, total_iters=args.num_epochs)
